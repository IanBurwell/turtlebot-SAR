...
def tag_detected(tag_ID, T_c_tag, T_w_tag, max_detect_dis, max_detect_angle, 
                 max_comp_dis, max_comp_angle, tag_dict, tag_queue):
    # Checks if detection is valid (not necessarily valid for composition)
    if aglomerate_apriltag_pose.check_valid_measurement(T_c_tag, max_detect_angle, 
                                                        max_detect_angle, max_detect_dis):
